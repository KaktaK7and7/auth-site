...
function ensureSocialSchema() {
  if (!socialSchemaPromise) {
    ...
    socialSchemaPromise = initSocialSchema(pool)
      ...
      .then(() => initGroupSchema(pool))
      .then(() => initFriendCodeSchema(pool))
      .then(() => initSubscriptionSchema(pool))
      .catch((error) => {
...
function sanitizeCommandCapabilities(value) {
  if (!Array.isArray(value)) return [];

  return value.slice(0, 80).flatMap((feature) => {
    if (!feature || typeof feature !== "object") return [];
    const featureId = String(feature.feature_id || "").trim().slice(0, 100);
    if (!/^[a-z0-9][a-z0-9._-]{0,99}$/i.test(featureId)) return [];

    const actions = Array.isArray(feature.actions)
      ? feature.actions.slice(0, 40).flatMap((action) => {
          if (!action || typeof action !== "object") return [];
          const actionId = String(action.action_id || "").trim().slice(0, 120);
          if (!/^[a-z0-9][a-z0-9._-]{0,119}$/i.test(actionId)) return [];
          const voiceExamples = Array.isArray(action.voice_examples)
            ? action.voice_examples
              .filter((example) => typeof example === "string" && example.trim())
              .slice(0, 8)
              .map((example) => String(example).trim().slice(0, 100))
            : [];
          return [{
            action_id: actionId,
            display_name: String(action.display_name || actionId).trim().slice(0, 160),
            argument_hint: String(action.argument_hint || "").trim().slice(0, 160),
            voice_examples: voiceExamples,
          }];
        })
      : [];

    return actions.length ? [{ feature_id: featureId, actions }] : [];
  });
}


async function sendSubscriptionCatalog(_req, res) {
  ...
  return res.json({
    ...
    billing: {
      model: "fixed_subscription_with_included_ai_resource",
      overage: "disabled",
      topups: "planned",
      local_snake_consumes_ai: false,
    },
  });
}


async function sendSubscriptionMe(req, res) {
  const pool = getSocialPool();
  ...
  try {
    const subscription = await getSubscriptionStatus(pool, user.id);
...
async function sendSemanticCommandRoute(req, res) {
  const pool = getSocialPool();
  const user = await resolveSocialUser(pool, req);
  if (!user) {
    return res.status(401).json({ ok: false, error: "Not authenticated" });
  ...
  const capabilities = sanitizeCommandCapabilities(req.body?.capabilities);
  if (!message || message.length > 2000 || !capabilities.length) {
    return res.status(400).json({ ok: false, error: "Invalid command route request" });
  }

  const baseUrl = String(process.env.AI_SERVICE_URL || "").trim().replace(/\/+$/, "");
  const internalToken = String(process.env.AI_INTERNAL_TOKEN || "").trim();
  if (!baseUrl || !internalToken) {
    return res.status(503).json({ ok: false, error: "Assistant service is not configured" });
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 12_000);
  try {
    const response = await fetch(`${baseUrl}/command-route`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Ziren-Internal-Token": internalToken,
      },
      body: JSON.stringify({
    ...
    if (!response.ok) {
      ...
      const subscriptionMessage = detail && typeof detail === "object"
        ? String(detail.message || "")
        : "";
      const subscriptionCode = detail && typeof detail === "object"
        ? String(detail.code || "")
        : "";
      return res.status(response.status >= 500 ? 502 : response.status).json({
        ok: false,
        error: subscriptionMessage || "Semantic command routing failed",
    ...
    return res.json({
      ...
      command_like: data?.command_like === true,
      feature_id: String(data?.feature_id || "").slice(0, 100),
      action_id: String(data?.action_id || "").slice(0, 120),
      arguments: data?.arguments && typeof data.arguments === "object"
        ? data.arguments
        : {},
    ...
    console.error("semantic command route error:", error);
    return res.status(error?.name === "AbortError" ? 504 : 502).json({
      ok: false,
      error: error?.name === "AbortError"
        ? "Semantic command routing timeout"
        : "Semantic command routing unavailable",
    });
  } finally {
...
function sendApiNotFound(req, res, next) {
  ...
  if (requestPath === "/subscriptions/catalog") {
    if (req.method !== "GET") {
      return res.status(405).json({ ok: false, error: "Method not allowed" });
    }
    return sendSubscriptionCatalog(req, res);
  }

  if (requestPath === "/subscriptions/me") {
    if (req.method !== "GET") {
      return res.status(405).json({ ok: false, error: "Method not allowed" });
    }
    return sendSubscriptionMe(req, res);
  }

  if (requestPath === "/assistant/command-route") {
    if (req.method !== "POST") {
      return res.status(405).json({ ok: false, error: "Method not allowed" });
    }
    return sendSemanticCommandRoute(req, res);
  }

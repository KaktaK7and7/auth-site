...
function networkProfileUrl(userId) {
  return `/network-profile.html?id=${Number(userId)}`;
}
    ...
    try {
      ...
      const profileResult = await pool.query(
        ...
        SELECT id, username, avatar_url, status_text, bio,
               public_profile_enabled, show_friends_on_profile, friend_code
        FROM users
        WHERE id = $1
        LIMIT 1
        `,
      ...
      const relationResult = targetId === Number(req.friendCodeUser.id)
        ? { rows: [] }
        : await pool.query(
            `
            ...
            LIMIT 1
            `,
            [req.friendCodeUser.id, targetId],
          );
      const relation = relationResult.rows[0] || null;

      let friends = [];
      let friendsCount = null;
      if (profile.show_friends_on_profile) {
        ...
        friendsCount = friendsResult.rowCount || 0;
        friends = friendsResult.rows.map((friend) => ({
          id: Number(friend.id),
          username: friend.username,
          avatar_url: friend.avatar_url || "/images/Ziren.png",
          network_profile_url: networkProfileUrl(friend.id),
        }));
      }

      const relationship = targetId === Number(req.friendCodeUser.id)
        ? { status: "self", request_direction: null }
        : relation
          ? {
              status: relation.status,
              request_direction:
                relation.status === "pending"
                  ? Number(relation.requested_by) === Number(req.friendCodeUser.id)
                    ? "outgoing"
                    : "incoming"
                  : null,
            }
          : { status: "none", request_direction: null };
      ...
      return res.json({
        ...
        profile: {
          ...
          public_profile_url: profile.public_profile_enabled
            ? `/community/${profile.id}`
            : null,
          friends_visible: Boolean(profile.show_friends_on_profile),
    ...
    try {
      ...
      return res.json({
        ...
        users: result.rows.map((row) => ({
          ...
          network_profile_url: networkProfileUrl(row.id),

...
function buildStoryGraphNodes() {
  ...
  Object.values(ROUTES).forEach((route) => {
    ...
    routeChoice.options.forEach((protocolOption, protocolIndex) => {
      const protocolY = route.y + (protocolIndex - 1) * 190;
...
function normalizeStoryState(rawState) {
  ...
  return {
    ...
    relationship: {
      ...
      closeness: Number(relationship.closeness)
        || Math.max(0, Math.floor((Number(relationship.trust) || 0) / 2)),

...
const FEATURE_LABELS = {
  "system.app_launcher": "Запуск приложений",
  "system.media_control": "Управление музыкой",
  "system.test": "Проверка модульной системы",
  "system.volume": "Управление громкостью",
  "system.window_control": "Управление окнами",
};
  ...
  return topCommands
    .map(
      (command) => `
        <div>
          <span>${escapeHtml(
            FEATURE_LABELS[command.command_text] || command.command_text,
          )}</span>
      ...
      <form class="profile-form" method="post" action="/profile/settings">
        ...
        <label class="profile-switch">
          ...
          <span>
            ...
            <span>Сохраняет идентификаторы выполненных функций для статистики, уровней и достижений.</span>
          </span>
        </label>
        ...
        <label class="profile-switch">
          ...
          <span>
            ...
            <span>Добавляет к разрешённому событию короткую распознанную формулировку команды, чтобы Мелисса могла естественно отреагировать. Экран и файлы не передаются.</span>
          </span>
        </label>

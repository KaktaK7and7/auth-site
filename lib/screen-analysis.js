...
function targetScore(query, label) {
  ...
  for (const queryToken of queryTokens) {
    if (labelTokens.some((labelToken) => {
      const prefixLength = Math.min(6, queryToken.length, labelToken.length);
      return prefixLength >= 3 && (
        queryToken.slice(0, prefixLength) === labelToken.slice(0, prefixLength)
        || queryToken.includes(labelToken)
        || labelToken.includes(queryToken)
      );
    })) {
      score += 1;
    }

...
function lexicalTargetConfidence(query, label) {
  ...
  const matchedLabels = labelTokens.filter((labelToken) => (
    queryTokens.some((queryToken) => tokensMatch(queryToken, labelToken))
  ));
  return matchedLabels.length / labelTokens.length;
}

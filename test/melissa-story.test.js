...
test("Melissa story starts with one unlocked memory and the first choice", () => {
  ...
  assert.equal(publicState.prologue.next_choice.id, "first_contact");
  assert.equal(publicState.nodes[0].status, "unlocked");
  assert.equal(publicState.nodes[1].status, "discovered");
});
...
test("story context stays compact and carries the selected name", () => {
  ...
  assert.match(context, /Текущее имя: Искра/);
  assert.match(context, /Романтическая ветка выключена/);
